...
export function ClientePage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const { data: cli } = await supabase.from('clientes').select('*').eq('id', invitacion.cliente_id).maybeSingle()
      const { data: veh } = await supabase.from('vehiculos').select('*').eq('id', invitacion.vehiculo_id).maybeSingle()
